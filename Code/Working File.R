...
dataQ3_b <- dataQ3_a %>%
  select(-REGION, -STATECENSUS, -PERNUM, -SEX, -MARST, - EMPSTAT, -LABFORCE, -DURUNEMP, -DURUNEM2, -WHYABSNT, 
         -WKSTAT, -MULTJOB, - NUMJOB, - UNION, -OTPAY, -ASECFLAG, -METRO, -SERIAL, -HISPAN, - EDUC99) %>%     # METRO = pain in ass with NA's, why?
...
# match with industry.csv.    ############################ Need to fix state_fips first.  ############################
dataQ3_b <- dataQ3_a %>%
  mutate(STATEFIP = as.character(STATEFIP) %>%
  left_join(state_fips, by = ('STATEFIP (FIPS)')))

dataQ3_merge <- dataQ3 %>%
  left_join(state_fips, by = 'STATEFIP') %>%    # IND = numberic in dataQ3
  left_join(Industries, by = 'IND') %>%
  mutate(Year = as.date(YEAR), Month = ymd(YMMONTH)) 
  
  mutate(IndustryName = as_factor(indname))

###### PRE-COVID DATA SUBSET
  
  
  
  
#### DURING COIVD DATA SUBSET
