...
#### Retail v Other Industries (Overview)
```{r, warning = FALSE, message = FALSE}
reg_df_vis %>%
  ...
  geom_vline(xintercept=as_date('2020-03-01'), color = "black", size=.5)+
  geom_vline(xintercept=as_date('2020-04-01'), color = "black", size=.5)+
  labs(x = "Survey Date", y = "Employment Rate",
    title ="Employment Rate (Retail v Other Industries)")+
  theme(aspect.ratio=3/7)
```
...
#   
#   demo_data %>%
#   group_by(METRO, indname) %>% 
#   count() %>% 
#   ggplot(aes(x=METRO, y = n, fill = indname))+geom_bar(stat = 'identity', position = 'fill')+
#   
#   theme(aspect.ratio=3/7)
# EMPC
#   summarize(ct = count(unique(CPSIDP)))
...
unemp_by_industry_retail <- demo_data %>% 
  group_by(period_type, date_, retail_v_other) %>%
  summarize(UnemploymentRate = 1- (weighted.mean(is_employed == TRUE, w = WTFINL)))%>%
  ggplot(aes(x = date_, y = UnemploymentRate, color = retail_v_other))+ geom_line()+
  geom_vline(xintercept=as_date('2020-03-01'),
                 color = "red", size=1)+
...
reg_df <- demo_data %>%
  select(period_type
         ...
         , WTFINL
         ## Variables
         , IND, indname, REGION, FamilyIncome
         ## Fixed
         , AGE, sex, RACE
  )
summary(reg_df)
...
reg2 <- reg_df %>%
  feols(is_employed~period_type+indname, se = 'hetero')

reg3 <- reg_df %>%
  feols(is_employed~period_type+REGION)

reg4 <- reg_df %>%
  feols(is_employed~indname + REGION + period_type)
...
<!-- ###################################################### -->
<!-- ######### Should get covered by regressions ######### -->
<!-- ###################################################### -->
...
<!-- vtable(EMP_AGG_Precovid_race_age) -->
<!-- %>% -->
<!--   ggplot(aes(x = date_, y = UnemploymentRate, color = cats2))+ geom_line()+ -->
<!--   geom_vline(xintercept=as_date('2020-03-01'), -->
<!--                 color = "red", size=1)+ -->
