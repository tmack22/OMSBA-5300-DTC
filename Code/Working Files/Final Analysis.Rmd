...
####First Regression
```{r}
reg1 <- feols(log(emp_count)~indname,se= 'hetero',data= filtered_df)
reg1.1 <- feols(log(emp_count)~indname + covid_active,se='hetero',data= filtered_df)
etable(reg1,reg1.1)
```
...
####Second Regression
```{r}
reg2 <- feols(log(emp_count)~industry_cat,se='hetero',data= filtered_df)
reg2.1 <- feols(log(emp_count)~industry_cat + covid_active,se='hetero',data= filtered_df)
etable(reg2,reg2.1)
```
This regression is similar to the first regression except it was on the industry category field that grouped all industries into a binary variable of 'Retail' and 'Non-Retail'. This once again showed that there is a positive correlation between retail employment count.
