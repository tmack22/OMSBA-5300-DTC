...
demo_data <- DF %>%
  ...
  mutate(WHYUNEMP = case_when(
    ...
    WHYUNEMP == 6 ~ 'FirstJob',
    WHYUNEMP == 0 ~ 'Military')) %>%
  mutate(employed = ifelse(EMPSTAT <20 & EMPSTAT >= 10 , 1, 0), unemployed = ifelse(EMPSTAT > 20 & EMPSTAT<= 22, 1, 0)) %>% # I updated the conditions to match IPUMS EMPSTAT definitions. 
