...
demo_data <- DF %>%
  ...
  select(date_, YEAR, MONTH, HHID, PERNUM, EMPSTAT, is_employed, employed, unemployed, CPSID, CPSIDP    # added employed and unemployed to select-EC
...
```{r, warning = FALSE, message = FALSE}
## All industries unemployment rate over time  - CORRECT!
...
# causal diagram - would be nice to illustrate our logic?
...
#reg1a interpretation:
#A one unit change in employment precovid is associated with a -.0077 decrease in employment during covid and statistically significant at the .01 perecent level

reg1a <- feols(is_employed~indname, se = 'hetero', data = reg_df_precovid)
etable(reg1a)

reg1b <- feols(is_employed~indname, se = 'hetero', data = reg_df_Covid)
etable(reg1b)


reg1c <- feols(is_employed~indname, se = 'hetero', data = reg_df_postcovid)
etable(reg1, reg1a, reg1b, reg1c)


Fixed effects notes, "so if we have fixed effects fo individual, we are comparing that individual to themselves over time.  Including
fixed effects for both individual and for time period is commonly know as tow-way fixed effects."


# causal diagram - would be nice to illustrate our logic?
reg2 <- feols(is_employed~indname, se = 'hetero', data = reg_df)
reg2a <- feols(is_employed~indname + period_type, se = 'hetero', data = reg_df)
etable(reg2, reg2a)

INTERPRETATION: 
controlling other variables in the model, a one-unit change in X  is associated with a B1 -unit change in Y
A one unit increase in employment is 


#### INTERPRETATION LANGUAGE :holding all else constant and compared to covid, a one-unit

# regression evaluating effect of RACE on employment
# insert causal diagram
reg3 <- feols(is_employed~RACE,  se = 'hetero', data = reg_df)
etable(reg3)

# regression evaluating effect of being in a categorical age group on employment
# insert causal diagram
reg4 <- feols(is_employed~AgeGroup + period_typ,  se = 'hetero', data = reg_df)
etable(reg4)

# regressing evaluating effect of marriage on 
reg5 <- feols(is_employed~MARST + AgeGroup + period_type,  se = 'hetero', data = reg_df)
etable(reg5)
